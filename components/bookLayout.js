...
export default function BookLayout({ children, title }) {
      ...
      <div data-theme={"studio9"}>
        <div className="mx-auto max-w-7xl py-4 sm:py-6 lg:py-8 px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between">
            ...
            <div className="m-6">
              <Link aria-label="go to cart" href="/cart" className="p-10">
                {cart.cartItems.length > 0 && cartItemsCount > 0 && (
                  <span className="ml-2 rounded-full bg-accent px-2 py-1 text-xs font-bold text-black">
                    {cartItemsCount}
                  </span>
                )}
                <ShoppingCartIcon />
          ...
          <div className="relative hidden md:block h-20 mt-24">
            <Image src={separator} alt={"separator"} fill sizes="100vw" />
          </div>
          <div className="relative md:hidden h-20 mt-24">
            <Image src={mobileSeparator} alt={"separator"} fill sizes="100vw" />
          </div>
          <Footer />
        </div>
      </div>
